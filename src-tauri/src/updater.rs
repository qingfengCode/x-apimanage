...
/// 下载进度事件名（前端 listen("update:progress")）。
...
///
/// 进度通过 `update:progress` 事件推送（节流后）。若清单提供 sha256，
/// 下载完成后校验；不一致则删除文件并报错。
pub async fn download(
    ...
    let file_name = manifest
        ...
        .filter(|s| !s.is_empty())
        .map(|s| s.to_string())
        .unwrap_or_else(|| format!("x-apimanage_{}_setup.exe", manifest.version));
...
/// 计算并广播一次下载进度。
fn emit_progress(app: &tauri::AppHandle, received: u64, total: u64) {
...
pub fn install_and_exit(app: &tauri::AppHandle, installer: &Path) -> AppResult<()> {
    if !installer.exists() {
        return Err(AppError::Update(format!(
            "安装包不存在: {}",
            installer.display()
